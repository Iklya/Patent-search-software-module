...
Выпускная квалификационная работа бакалавра по направлению «Информатика и вычислительная техника» (ВолгГТУ, 2026 г.).  
...
Дополнительно к модульному тестированию была проведена экспериментальная оценка качества поиска патентов-аналогов на полнотекстовой патентной базе объемом **100 000 документов**.

#### Методика тестирования

Для тестирования использовалась выборка из **50 патентов**. Для каждого исходного патента в качестве релевантного патента-аналога выбирался **первый патент из списка цитируемых документов Google Patents**. Поиск выполнялся по автоматически извлеченным ключевым фразам (в среднем около **7 фраз** на запрос).

Помимо стандартных логических режимов `AND` и `OR`, дополнительно исследовались промежуточные режимы поиска с использованием параметра `minimum_should_match` (MSM) Elasticsearch, позволяющего задавать минимальную долю совпадающих ключевых фраз.

#### Используемые метрики

| Метрика | Описание |
|---------|----------|
| **HR@10** | Доля запросов, в которых релевантный патент найден среди первых 10 результатов |
| **HR@25** | Доля запросов, в которых релевантный патент найден среди первых 25 результатов |
| **HR@50** | Доля запросов, в которых релевантный патент найден среди первых 50 результатов |
| **HR@All** | Доля запросов, в которых релевантный патент найден независимо от позиции |
| **AvgPos** | Средняя позиция релевантного патента в поисковой выдаче (чем меньше, тем лучше) |

#### Результаты тестирования

| Режим поиска | HR@10 | HR@25 | HR@50 | HR@All | AvgPos |
|-------------|-------|-------|-------|--------|--------|
| OR | 0.42 | 0.56 | 0.68 | 0.82 | 7.24 |
| MSM 10% | 0.42 | 0.56 | 0.68 | 0.82 | 7.24 |
| MSM 20% | 0.42 | 0.56 | 0.68 | 0.82 | 7.24 |
| MSM 30% | 0.42 | 0.56 | 0.68 | 0.82 | 7.24 |
| MSM 40% | 0.42 | 0.54 | 0.66 | 0.80 | 7.32 |
| MSM 50% | 0.42 | 0.54 | 0.66 | 0.80 | 7.31 |
| MSM 60% | 0.36 | 0.50 | 0.60 | 0.72 | 7.98 |
| MSM 70% | 0.30 | 0.42 | 0.44 | 0.52 | 9.76 |
| MSM 80% | 0.26 | 0.38 | 0.42 | 0.48 | 10.87 |
| MSM 90% | 0.14 | 0.18 | 0.20 | 0.20 | 31.20 |
| AND | 0.14 | 0.18 | 0.20 | 0.20 | 24.76 |

#### Визуализация результатов

##### Зависимость Hit Rate от значения MSM

![Зависимость Hit Rate от MSM](docs/hr_vs_msm.png)

На графике показано, как меняются показатели HR@10, HR@25, HR@50 и HR@All при увеличении значения MSM от 0 (OR) до 1.0 (AND). Наблюдается устойчивое снижение всех метрик при MSM > 0.5, что свидетельствует о том, что избыточные требования к совпадению ключевых фраз приводят к потере релевантных документов.

##### Зависимость средней позиции релевантного документа от MSM

![Зависимость AvgPos от MSM](docs/avgpos_vs_msm.png)

График демонстрирует, что при MSM ≤ 0.5 средняя позиция релевантного патента остаётся в пределах 7–8, тогда как при MSM ≥ 0.7 наблюдается резкий рост AvgPos, достигающий значений 25–31 для режимов AND и MSM 90%.

#### Анализ результатов

**Наилучшие показатели** достигаются при использовании режимов `OR` и `MSM 10–50%`:
- релевантный патент находится в **82%** случаев;
- средняя позиция составляет **~7**, что означает нахождение целевого документа вблизи начала поисковой выдачи.

**Наихудшие показатели** демонстрируют режимы `AND` и `MSM 90%`:
- релевантный патент находится лишь в **20%** случаев;
- средняя позиция возрастает до **25–31**, что делает поиск практически неэффективным.

**Причина низкой эффективности строгих режимов** — высокая вариативность патентной терминологии. Даже тематически близкие патенты часто описывают сходные технические решения разными терминами. В таких условиях требование совпадения большинства или всех ключевых фраз (`AND`, высокие MSM) приводит к исключению релевантных документов из поисковой выдачи.

#### Выводы

1. Режим `OR` обеспечивает оптимальный баланс между полнотой и точностью поиска патентов-аналогов.
2. Параметр MSM в диапазоне 10–50% не ухудшает качество поиска, что подтверждает устойчивость системы.
3. Режим `AND` не рекомендуется для полнотекстового патентного поиска вследствие высокой вариативности формулировок в патентных документах.
4. Полученные метрики (HR@10 = 0.42, HR@All = 0.82, AvgPos = 7.24) подтверждают практическую применимость разработанного модуля для задач патентного поиска.
