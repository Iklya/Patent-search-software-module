...
|-------------|-------|-------|-------|--------|--------|
| OR | 0.36 | 0.54 | 0.56 | 0.80 | 9.03 |
| MSM 10% | 0.36 | 0.54 | 0.56 | 0.80 | 9.03 |
| MSM 20% | 0.36 | 0.54 | 0.56 | 0.80 | 9.03 |
| MSM 30% | 0.36 | 0.54 | 0.56 | 0.80 | 9.03 |
| MSM 40% | 0.36 | 0.52 | 0.54 | 0.76 | 9.13 |
| MSM 50% | 0.36 | 0.52 | 0.54 | 0.76 | 9.12 |
| MSM 60% | 0.32 | 0.46 | 0.48 | 0.72 | 9.82 |
| MSM 70% | 0.26 | 0.38 | 0.42 | 0.52 | 12.22 |
| MSM 80% | 0.22 | 0.34 | 0.38 | 0.48 | 13.20 |
| MSM 90% | 0.10 | 0.16 | 0.18 | 0.20 | 34.22 |
| AND | 0.10 | 0.16 | 0.18 | 0.20 | 27.11 |
...
График демонстрирует, что при MSM ≤ 0.5 средняя позиция релевантного патента остаётся в пределах 9, тогда как при MSM ≥ 0.7 наблюдается резкий рост AvgPos, достигающий значений 27–34 для режимов AND и MSM 90%.

#### Анализ результатов

**Наилучшие показатели** достигаются при использовании режимов `OR` и `MSM 10–50%`:
- релевантный патент находится в **80%** случаев;
- средняя позиция составляет **~9**, что означает нахождение целевого документа вблизи начала поисковой выдачи.

**Наихудшие показатели** демонстрируют режимы `AND` и `MSM 90%`:
- релевантный патент находится лишь в **20%** случаев;
- средняя позиция возрастает до **27–34**, что делает поиск практически неэффективным.
...
4. Полученные метрики (HR@10 = 0.36, HR@All = 0.80, AvgPos = 9.03) подтверждают практическую применимость разработанного модуля для задач патентного поиска.
